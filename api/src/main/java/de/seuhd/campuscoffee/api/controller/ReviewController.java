...
public class ReviewController extends CrudController<Review, ReviewDto, Long> {
    ...
    public ResponseEntity<List<ReviewDto>> filter(
            @Parameter(description="ID of the point of sale to retrieve reviews for.", required=true)
            @RequestParam("pos_id") Long posId,
            @RequestParam("approved") Boolean approved) {
                List<ReviewDto> reviews = reviewService.filter(posId, approved).stream()
    ...
    public @NonNull ResponseEntity<ReviewDto> approve(
            ...
            @PathVariable Long id,
            @Parameter(description="ID of the user approving the review.", required=true)
